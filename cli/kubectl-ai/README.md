# Integrating Kthena CLI with kubectl‑ai

Enhance your Kubernetes AI workflow by integrating **Kthena CLI** with **kubectl‑ai** – an LLM‑powered tool that suggests and executes Kubernetes operations using a curated set of tools.

## Overview

**[kubectl‑ai](https://github.com/GoogleCloudPlatform/kubectl‑ai)** is an open‑source plugin that uses large language models to understand natural‑language prompts and generate appropriate Kubernetes commands. It comes with built‑in tools like `kubectl` and `bash`, but can be extended with custom tools.

**Kthena CLI** (`kthena`) is a Kubernetes‑native command‑line interface for managing AI inference workloads, model deployments, autoscaling policies, and templates. By integrating Kthena as a custom tool, you enable kubectl‑ai to perform advanced AI‑serving operations – from deploying DeepSeek or Qwen models to managing scaling policies – using simple English prompts.
...
3. **Place the Kthena tool configuration**  
   By default, kubectl‑ai looks for tool configurations in `~/.config/kubectl‑ai/tools.yaml`. Copy the provided [`tools.yaml`](./tools.yaml) to that location:
   ```bash
...
The [`tools.yaml`](./tools.yaml) file defines how kubectl‑ai invokes the `kthena` command. It includes:
...
When kubectl‑ai processes a prompt, it will match the intent to one of the described subcommands and generate the appropriate `kthena` invocation.
